...
optimizer = optim.Adam(network.parameters(), lr = learningRate, weight_decay = 0.0005)
scheduler = lr_scheduler.MultiStepLR(optimizer,milestones=[4,9,15,25],gamma=0.1);

##########################################################################
# START TRAINING #########################################################
##########################################################################

train_model(network, criterion, optimizer, scheduler, train_loader, val_loader, n_epochs = 30, use_gpu = True)
torch.save(network, 'vgg16_30e_5e-4.pth')
